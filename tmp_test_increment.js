
const admin = require('firebase-admin');
const projectId = 'firestoredb-6d462';
const clientEmail = 'firebase-adminsdk-tvqr4@firestoredb-6d462.iam.gserviceaccount.com';
const privateKey = "-----BEGIN PRIVATE KEY-----\\\\nJPZ6I7xVJXq6DhMLl5Sy4rg2\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n');

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      projectId,
      clientEmail,
      privateKey,
    }),
  });
}

const db = admin.firestore();

async function testGemsIncrement() {
  const userId = 'SWeAdl3RRIREvq2UpvtpwgAemsr1'; // Ahmad
  const xpGained = 50;
  const gemsGained = 50;
  const stage = 'test-stage-' + Date.now();

  const userRef = db.collection('users').doc(userId);
  const before = await userRef.get();
  const gemsBefore = before.data().gems || 0;

  console.log(`Gems before: ${gemsBefore}`);

  await userRef.update({
    gems: admin.firestore.FieldValue.increment(gemsGained),
    completedMissions: admin.firestore.FieldValue.arrayUnion(stage)
  });

  const after = await userRef.get();
  const gemsAfter = after.data().gems || 0;

  console.log(`Gems after: ${gemsAfter}`);
  
  if (gemsAfter === gemsBefore + gemsGained) {
    console.log("SUCCESS: Gems incremented correctly!");
  } else {
    console.log("FAILURE: Gems did NOT increment correctly!");
  }
}

testGemsIncrement().catch(console.error);
