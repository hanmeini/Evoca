
const admin = require('firebase-admin');
const projectId = 'firestoredb-6d462';
const clientEmail = 'firebase-adminsdk-tvqr4@firestoredb-6d462.iam.gserviceaccount.com';
const privateKey = "-----BEGIN PRIVATE KEY-----\\\\nJPZ6I7xVJXq6DhMLl5Sy4rg2\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n');

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      projectId,
      clientEmail,
      privateKey,
    }),
  });
}

const db = admin.firestore();

const MISSION_REWARDS = {
  'daily-visit': 10,
  'm1': 50,
  'm2': 30,
  'm4': 40,
  'm3': 100,
  'achievement-quiz-perfect': 100,
  'a-bronze-coll': 100
};

async function sanitizeUserXP(userId) {
  const userRef = db.collection('users').doc(userId);
  const userDoc = await userRef.get();
  if (!userDoc.exists) return;

  const userData = userDoc.data();
  let trueXP = 0;

  // 1. Calculate from missions
  if (userData.completedMissions) {
    userData.completedMissions.forEach(missionKey => {
      // Keys are claim-YYYY-MM-DD-id or claim-id
      const idMatch = missionKey.match(/claim-(?:\d{4}-\d{2}-\d{2}-)?(.+)/);
      if (idMatch) {
        const id = idMatch[1];
        trueXP += MISSION_REWARDS[id] || 0;
      }
    });
  }

  // 2. Calculate from documents
  const docsSnapshot = await db.collection('documents').where('userId', '==', userId).get();
  docsSnapshot.forEach(doc => {
    const data = doc.data();
    if (data.completedStages) {
      data.completedStages.forEach(stage => {
        if (stage === 'podcast') trueXP += 50;
        if (stage === 'chat') trueXP += 50;
        if (stage === 'quiz') {
          // Quiz XP depends on score, but we'll assume max if not stored perfectly
          // or just 100 as a baseline if we don't have the score in the stage string
          trueXP += 100; 
        }
      });
    }
  });

  console.log(`User: ${userData.displayName} | Old XP: ${userData.totalXP} | True XP: ${trueXP}`);
  
  // Update to True XP
  await userRef.update({ totalXP: trueXP });
}

async function runSanity() {
  const users = await db.collection('users').get();
  for (const doc of users.docs) {
    await sanitizeUserXP(doc.id);
  }
}

runSanity().catch(console.error);
